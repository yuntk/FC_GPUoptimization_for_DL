...
with tf.Session(config=session_config) as sess:
  ...
  for step in range(max_steps):
    ...
    st = time.time()
    _, loss = sess.run([train_step, cross_entropy], feed_dict={x: batch_xs, y_: batch_ys, keep_prob: 0.5},options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE),
...
with open('./single_mnist.json', 'w') as file:
        file.write(trace.generate_chrome_trace_format())
